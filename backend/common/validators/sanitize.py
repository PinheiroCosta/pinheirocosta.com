...
def validate_no_html(field_name: str, value: str) -> str:
    ...
    if contains_html(value):
        raise serializers.ValidationError(f"HTML não é permitido no campo '{field_name}'.")
    return value
...
def contains_sql_keywords(text: str) -> bool:
    ...
    """
    sql_payloads = [
        "DROP TABLE", "DELETE FROM", "INSERT INTO", "UPDATE", "SELECT * FROM",
...
def validate_no_sql_injection(field_name: str, value: str) -> str:
    ...
    if contains_sql_keywords(value):
        raise serializers.ValidationError(f"Entrada suspeita de SQL injection no campo '{field_name}'.")
    return value
